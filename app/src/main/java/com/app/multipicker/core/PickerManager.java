package com.app.multipicker.core;
...
public abstract class PickerManager {
   ...
   * <p/>
   * If you are setting the (@link CacheLocation#EXTERNAL_STORAGE_PUBLIC_DIR} make sure you have the
   * required permissions available in the Manifest file. Else, a {@link RuntimeException} will be
   * raised.
   * <p/>
   * Permissions required {@link Manifest.permission#WRITE_EXTERNAL_STORAGE} and {@link
  ...
  /**
   * This method should be called after  is
   * called.
   */
  ...
  private void checkIfPermissionsAvailable() {
    ...
    LogUtils.d(TAG, "checkIfPermissionsAvailable: In Manifest(WRITE_EXTERNAL_STORAGE): "
        + writePermissionInManifest);
    boolean readPermissionInManifest =
        getContext().checkCallingOrSelfPermission(Manifest.permission.READ_EXTERNAL_STORAGE)
            == PackageManager.PERMISSION_GRANTED;
    LogUtils.d(TAG, "checkIfPermissionsAvailable: In Manifest(READ_EXTERNAL_STORAGE): "
        + readPermissionInManifest);

    if (!writePermissionInManifest || !readPermissionInManifest) {
      if (!writePermissionInManifest) {
        LogUtils.e(TAG,
            Manifest.permission.WRITE_EXTERNAL_STORAGE + " permission is missing in manifest file");
      }
      if (!readPermissionInManifest) {
        LogUtils.e(TAG,
            Manifest.permission.READ_EXTERNAL_STORAGE + " permission is missing in manifest file");
      }
  ...
  protected String getNewFileLocation(String extension, String type) throws PickerException {
    ...
    file = new File(
        file.getAbsolutePath() + File.separator + UUID.randomUUID().toString() + "." + extension);
